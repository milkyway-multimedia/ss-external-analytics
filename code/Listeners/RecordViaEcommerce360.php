...
{
    /* @var string */
    public $url = 'https://<dc>.api.mailchimp.com/2.0/ecomm/order-add.json';

    public function ecommerce(Event $e = null, $queueName = '', $params = [])
    {
        $results = [];

        if (isset($params['action']) && !in_array($params['action'], ['transaction'])) {
            return [];
        }
        ...
        singleton('ea')->executeDrivers(function ($driver, $id) use (&$results, $params) {
            ...
            if (($driver instanceof Driver) && $apiKey = $driver->setting($id, 'ApiKey')) {
                $paramDefaults = [
                    'total'      => singleton('env')->get('Ecommerce360|Mailchimp|ExternalAnalytics.conversion_value',
                        0.00),
